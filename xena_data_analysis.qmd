...
```{r}
# Non log in positive numerical values is required to run in cibersort. 
...
for (c_type in solidtumors){
  ...
  sample_ids <- intersect(colnames(gExp_mat_solidtumor), unlist(sample_id_solidtumor[[c_type]], use.names = FALSE))
  gene_expr_df <- gExp_mat_solidtumor[,sample_ids]
  ...
  as_tibble(predicted_matrix) %>% 
    ...
    relocate(genes) %>% 
    write_tsv(file=paste0(file_dest, "/CIBERSORT-Adjusted_gene_expression_", c_type, ".tsv"))
