...
## Gene expression across cohorts
...
full_df <- full_df %>% 
  group_by(cancer_type) %>% 
    mutate(cancer_name = paste0(cancer_type, " n=", n_distinct(sample))) %>% 
    ungroup() %>% 
  mutate(g_name = ifelse(target == "CD274", "CD274 (PD-L1)",
                         ifelse(target == "PDCD1LG2", "PDCD1LG2 (PD-L2)", 
                                ifelse(target == "CCL2", "CCL2 (MCP-1)",
                                       ifelse(target == "BIRC7", "BIRC7 (MLIAP)",
                                              ifelse(target == "LRRC32", "LRRC32 (GARP)",
                                                     ifelse(target == "TDO2", "TDO2 (TDO)", target))))))) 
...
```{r}
target_x = targets_in_df_list[2]

get_box_stats <- function(y, upper_limit = max(filter(full_df, target==target_x)$gene_exp) * 1.5) {
  return(data.frame(
    y = 0.95 * upper_limit,
    label = paste(
      "n =", length(y)
...
#    stat_summary(fun.data = get_box_stats, geom = "text", hjust = 0.5, vjust = 0.9, size= 3.5) +
    ...
    facet_wrap(~ cancer_name, scales="free") +
...
```{r}
# Take gene_exp data used from before and 
```
