...
for (c_type in cancer_abbr_list) {
  ...
  if (!exists("mut_ctype")){
    ...
    mut_ctype[[c_type]] <- mut_ctype[[c_type]] %>% 
      filter(rowSums(select(., -sample) == 1) >= (0.1*num_cols))
...
ggbiplot::ggbiplot(pca_analysis, 
                   groups = dge$samples$neoplasm_histologic_grade, 
                   ellipse = TRUE,
                   var.axes = FALSE)
...
dge$samples$radiation_therapy

design_mat <- model.matrix(~0 + radiation_therapy, data=dge$samples)

# Remove some of the columns (discrepancy or no data)
design_mat <- design_mat %>% 
  as.data.frame() %>% 
  select(radiation_therapyNO, radiation_therapyYES) %>% 
...
```{r}
voomData <- voom(dge, design = design_mat, plot = TRUE)
```
...
contrasts_mat <- makeContrasts(
  ...
  radiation_therapy = radiation_therapyNO  - radiation_therapyYES
)
contrasts_mat
```
