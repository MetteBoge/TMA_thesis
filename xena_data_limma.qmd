...
if ("age_at_initial_pathologic_diagnosis" %in% colnames(df_pheno_of_interst)){
  ...
  q1_age <- stat_age["1st Qu."] %>% as.numeric()
  median_age <- stat_age["Median"] %>% as.numeric()
  q3_age <- stat_age["3rd Qu."] %>% as.numeric()
...
R_element_name <- ifelse(c_type == "SKCM", 
                         paste0(file_path_xenadata, 'limma_univariate/limma_univariate_', c_type, '_', sub_ctype, '.RData'),
...
table_w_adjP_sig %>% 
  ...
  mutate(phenotype = ifelse(grepl("mut_", phenotype), 
    ...
    mutate(phenotype = gsub("age_at_initial_pathologic_diagnosis", 
                             "Age at Diagnosis", phenotype)) %>% 
  rowwise() %>% 
