...
```{r}
res_quantiseq_AML <- deconvolute(gene_expr_matrix_AML, "quantiseq", tumor = TRUE)
res_quantiseq_SCM <- deconvolute(gene_expr_matrix_SCM, "quantiseq", tumor = TRUE)
res_quantiseq_PAD <- deconvolute(gene_expr_matrix_PAD, "quantiseq", tumor = TRUE)
```
...
```{r}
# Run the function - Takes a long time ro run so take a coffee break!
predicted_gene_expr_AML <- predict_all_genes(gene_expr_df_AML, quantiseq_AML)
predicted_gene_expr_PAD <- predict_all_genes(gene_expr_df_PAD, quantiseq_PAD)
predicted_gene_expr_SCM <- predict_all_genes(gene_expr_df_SCM, quantiseq_SCM)
```

Plot the gene expression of all targets in the AML data set and compare predicted to original:

```{r}
plt_predicted <- as_tibble(predicted_gene_expr_PAD) |>
...
plt_original <- as_tibble(gene_expr_matrix_PAD) |>
  mutate(gene = rownames(gene_expr_matrix_PAD)) |>
  dplyr::select(gene, everything()) |>
  filter(gene %in% target_list) |>
  mutate(across(-1, ~ log2(. + 0.001))) |>
  gather(sample, gene_expr, -gene) |>
