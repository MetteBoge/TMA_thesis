...
df_pheno_of_interst <- df_pheno_of_interst[,colSums(is.na(df_pheno_of_interst))<nrow(df_pheno_of_interst)]
```
...
make_all_contrasts <- function (group, delim="_vs_"){
  ...
  # Make sure MUT and WT are in right order for comparison. 
...
```{r}
# Check if the number of unique conditions matches the length of col_list
unique_conditions <- unique(table_w_adjP_sig$condition)

# Create a color vector that matches the length of unique conditions
col_list <- ifelse(unique_conditions %in% table_w_adjP_sig$condition[table_w_adjP_sig$sample_dist == "LOW"], "red", "black")
...
table_w_adjP_sig %>% 
  ...
  theme(axis.text.x = element_text(size = 14, 
                                   angle = 30),
    axis.text.y = element_text(colour = col_list,
                               size = 14)
  )
...
# Check if the number of unique conditions matches the length of col_list
unique_conditions <- unique(table_w_only_regP_sig$condition)

# Create a color vector that matches the length of unique conditions
col_list <- ifelse(unique_conditions %in% table_w_only_regP_sig$condition[table_w_only_regP_sig$sample_dist == "LOW"], "red", "black")

# Plot with correct y-axis coloring
table_w_only_regP_sig %>% 
  mutate(significance = case_when(
    P.Value < 0.001 ~ "***",
    P.Value < 0.01  ~ "**",
    P.Value > 0.05  ~ "*",
