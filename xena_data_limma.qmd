...
df_pheno_of_interst <- df_pheno_of_interst %>% 
  ...
  mutate_all(~replace(., . == "Not_Classified", NA))
...
if("histological_type" %in% colnames(df_pheno_of_interst)) {
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    ...
    mutate(histological_type = gsub("\\)", "", histological_type)) %>% 
    mutate(histological_type = make.names(histological_type))
}
...
df_pheno_of_interst <- data.frame(lapply(df_pheno_of_interst, function(column) {
  ...
  # If there were warnings or NAs, keep as character
  if (any(is.na(num_col)) & any(!is.na(column))) {
    as.character(column)
  } else {
    num_col
  }
...
table_w_adjP_sig %>% 
  mutate(condition = ifelse(phenotype == "histological_type", 
                            ...
                            condition),
         condition = ifelse(nchar(condition) > 100, 
                           str_replace(condition, "_vs_", "_vs_\n"), 
                           condition)) %>% 
...
if (qr(design_mat2)$rank < ncol(design_mat2)) {
  ...
  X_categorical <- X[ , !(colnames(X) %in% continous_sig_phenos)] %>% 
    as.data.frame() %>% 
    mutate_if(is.character, as.factor)
  ...
  cont_X <- X[,continous_sig_phenos] %>% as.data.frame()
...
```{r}
# Function to split string based on matches in data frame
split_string_by_df <- function(input_string, df) {
  for (phenotype in unique(df$phenotype)) {
    if (str_detect(input_string, fixed(phenotype))) {
      remaining <- str_remove(input_string, fixed(phenotype))
      cat_match <- df %>% filter(phenotype == !!phenotype) %>% pull(cat)
      for (cat in cat_match) {
        if (str_detect(remaining, fixed(cat))) {
          return(c(phenotype, cat))
        }
      }
    }
  }
...
```{r}
# Add category sample size to mutlivariate result table: 
multivariate_resuaslts 
...
```{r}
phenotypes_multivariate_w_adjP_sig <- multivariate_w_adjP_sig %>% 
  dplyr::select(phenotype) %>% 
  unlist(use.names = F) %>% 
  unique()

x_pheno <- phenotypes_multivariate_w_adjP_sig[1]
x_cond <- "Positive"

# Extract phenotype table with samples that od not have NA for phenotype we investigate: 
tmp_pheno_tbl <- dge$samples[!is.na(dge$samples[[x_pheno]]),]

samples_ER_pos <- tmp_pheno_tbl[tmp_pheno_tbl[,x_pheno] == x_cond,] %>% 
  rownames()

samples_ER_neg <- tmp_pheno_tbl[tmp_pheno_tbl[,x_pheno] == "Negative",] %>% 
  rownames()

samples_w_data <- rownames(tmp_pheno_tbl)

# List of genes that are significant up/down regulated in mnultivariate analysis for this phenotype
sig_genes <- multivariate_w_adjP_sig %>% 
  filter(adj.P.Val<0.05,
         phenotype == x_pheno) %>% 
  dplyr::select(gene) %>% 
  unlist(use.names = F)

# Gene expression table for only target genes with all samples that have data for x_pheno
gene_exp_all <- dge$counts %>% 
  as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  filter(!str_detect(gene, "^ENSG")) %>% 
  dplyr::select(gene, 
                all_of(intersect(samples_w_data, colnames(dge$counts)))) %>% 
  pivot_longer(cols = -gene,
               names_to = "samples",
               values_to = "gene_expression") %>% 
  mutate(phenotype = x_pheno,
         condition = ifelse(samples %in% samples_ER_pos,
                            x_cond,
                            ifelse(samples %in% samples_ER_neg, 
                                   "Negative",
                                   NA)))

# Gene expression table for only target genes with all samples that have data for all significant phenotypes (from univariate)
gene_exp_filtered <- dge_filtered$counts %>% 
  as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  filter(!str_detect(gene, "^ENSG")) %>% 
  dplyr::select(gene, 
                all_of(intersect(samples_w_data, colnames(dge_filtered$counts)))) %>% 
  pivot_longer(cols = -gene,
               names_to = "samples",
               values_to = "gene_expression") %>% 
  mutate(phenotype = x_pheno,
...
gene_exp_all %>% 
  filter(gene %in% sig_genes,
         !is.na(condition)) %>% 
  mutate(gene_expression = log2(gene_expression + 1)) %>% 
  ggplot(aes(x = gene, y = gene_expression, fill = condition)) +
  geom_boxplot()
...
gene_exp_all %>% dplyr::select(condition) %>%  unique()
```
