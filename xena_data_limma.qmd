...
# Not biological relevant                 "gender",
...
# Only one had YES                       "history_of_neoadjuvant_treatment",
                                         ...
                                         "pathologic_stage"
# Dont know before/after sample          "radiation_therapy"
                                        ),
                              "BRCA" = c("ER_Status_nature2012",
# This is same as gender                 "Gender_nature2012",
                                         "HER2_Final_Status_nature2012",
# Dont know before/after sample          "additional_pharmaceutical_therapy",
# Dont know before/after sample          "additional_radiation_therapy",
                                         "age_at_initial_pathologic_diagnosis",    # This is same as ER_Status_nature2012 "breast_carcinoma_estrogen_receptor_status",
# Morten says remove                     "er_level_cell_percentage_category",
# Only using female samples              "gender",
                                         "histological_type",
                                         "history_of_neoadjuvant_treatment",
# Morten says remove                     "margin_status",
                                         "PR_Status_nature2012", # I added this myself!!!! (to make triple negative...) 
                                         "menopause_status",
                                         "pathologic_stage"
# Dont know before/after sample          "radiation_therapy",
# Dont know before/after sample          "targeted_molecular_therapy"
                                          ),
                              "CESC" = c(
# Dont know before/after sample          "additional_pharmaceutical_therapy",
# Dont know before/after sample          "additional_radiation_therapy",
#                                        "adjuvant_rad_therapy_prior_admin",
                                         "age_at_initial_pathologic_diagnosis",
# Dont know before/after sample          "brachytherapy_administered_status",
                                         "clinical_stage",
                                         "eastern_cancer_oncology_group",
# Not biological relevant                "gender",
                                         "histological_type",
...
# Not biological relevant                 "gender",
# Dont use for LUAD, LUSC                 "histological_type",
                                         "history_of_neoadjuvant_treatment",
# Dont use anymore Morten                "karnofsky_performance_score",
...
# Only use extremes
if ("acute_myeloid_leukemia_calgb_cytogenetics_risk_category" %in% colnames(df_pheno_of_interst)){
  age_med <- median(df_pheno_of_interst$age_at_initial_pathologic_diagnosis)
  
  df_pheno_of_interst <- df_pheno_of_interst %>% 
     mutate(acute_myeloid_leukemia_calgb_cytogenetics_risk_category = ifelse(acute_myeloid_leukemia_calgb_cytogenetics_risk_category %in% c("Favorable", "Poor"),
                                                                             acute_myeloid_leukemia_calgb_cytogenetics_risk_category, NA)
     )
} 

# Split on median
if ("age_at_initial_pathologic_diagnosis" %in% colnames(df_pheno_of_interst)){
  age_med <- df_pheno_of_interst$age_at_initial_pathologic_diagnosis %>% 
    as.integer() %>% na.omit() %>% 
    median()
  
  df_pheno_of_interst <- df_pheno_of_interst %>% 
     mutate(age_at_initial_pathologic_diagnosis =  case_when(
      age_at_initial_pathologic_diagnosis < age_med ~ paste0("less_than_", age_med),
      age_at_initial_pathologic_diagnosis >= age_med ~ paste0("more_than_or_eq_",
                                                              age_med)))
} 

# Only use most extreme
if("alcoholic_exposure_category" %in% colnames(df_pheno_of_interst)) {
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    ...
    mutate(alcoholic_exposure_category = ifelse(alcoholic_exposure_category %in% c("None", "Daily_Drinker"),
                                                alcoholic_exposure_category, NA))
}
...
if ("breslow_depth_value" %in% colnames(df_pheno_of_interst)){
  ...
  if (sub_ctype == "SKCM_tumor"){
    bres_med <- median(df_pheno_of_interst$breslow_depth_value)
    
    df_pheno_of_interst <- df_pheno_of_interst %>% 
       mutate(breslow_depth_value =  case_when(
        breslow_depth_value < bres_med ~ paste0("less_than_", bres_med),
        breslow_depth_value >= bres_med ~ paste0("more_than_or_eq_", bres_med)))
...
if("clinical_stage" %in% colnames(df_pheno_of_interst)) {
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(clinical_stage = case_when(clinical_stage %in% c("Stage_I",
                                                            "Stage_IA", 
                                                            "Stage_IA1",
                                                            "Stage_IA2",
                                                            "Stage_IB",
                                                            "Stage_IB1",
                                                            "Stage_IB2") ~ "Stage_I",
                                      clinical_stage %in% c("Stage_II",
                                                            "Stage_IIA", 
                                                            "Stage_IIA1",
                                                            "Stage_IIA2",
                                                            "Stage_IIB") ~ "Stage_II",
                                      clinical_stage %in% c("Stage_III",
                                                            "Stage_IIIA",
                                                            "Stage_IIIB") ~ "Stage_III",
                                      clinical_stage %in% c("Stage_IV",
                                                            "Stage_IVA",
                                                            "Stage_IVB") ~ "Stage_IV"))
  # Clinical stage: 
  # CESC: Remove stage II
  # HNSC: I/II vs IV (remove III)
  ...
  if (c_type == "DLBC"){
     df_pheno_of_interst <- df_pheno_of_interst %>% 
       mutate(clinical_stage = case_when(clinical_stage %in% c("Stage_I",
                                                               "Stage_II") ~ "Stage_I_II",
                                      clinical_stage %in% c("Stage_III",
                                                            "Stage_IV") ~ "Stage_III_IV"))
  }
  
  if (c_type == "CESC"){
     df_pheno_of_interst <- df_pheno_of_interst %>% 
       mutate(clinical_stage = case_when(clinical_stage %in% c("Stage_III",
                                                            "Stage_IV") ~ "Stage_III_IV",
                                         TRUE ~ clinical_stage)) %>% 
       mutate(clinical_stage = ifelse(clinical_stage == "Stage_II", NA, clinical_stage))
  }
  ...
  if (c_type == "HNSC"){
     df_pheno_of_interst <- df_pheno_of_interst %>% 
       mutate(clinical_stage = case_when(clinical_stage %in% c("Stage_I",
                                                               "Stage_II") ~ "Stage_I_II",
                                         TRUE ~ clinical_stage)) %>% 
       mutate(clinical_stage = ifelse(clinical_stage == "Stage_III", NA, clinical_stage))
  }
...
if("eastern_cancer_oncology_group" %in% colnames(df_pheno_of_interst)) {
  ...
  other_ecog <- df_pheno_of_interst[["eastern_cancer_oncology_group"]] %>% 
    na.omit() %>% unique() %>% as.numeric() %>% sort()
...
if ("melanoma_clark_level_value" %in% colnames(df_pheno_of_interst)){
  # Do not use breslowth depth value for metastatic melanoma.
  if (sub_ctype == "SKCM_metastatic"){
    # Keep extremes (I and II vs V)
    df_pheno_of_interst <- df_pheno_of_interst %>% 
      mutate(melanoma_clark_level_value = case_when(melanoma_clark_level_value %in% c("I",
                                                                                      "II") ~ "I_II",
                                                    melanoma_clark_level_value %in% c("III",
                                                                                      "IV") ~ NA,
                                                    TRUE ~ melanoma_clark_level_value))
    
    #df_pheno_of_interst <- df_pheno_of_interst %>% 
    #  mutate(melanoma_clark_level_value = case_when(melanoma_clark_level_value %in% c("I",
    #                                                                                  "II", 
    #                                                                                  "III") ~ "I_II_III",
    #                                                melanoma_clark_level_value %in% c("IV",
    #                                                                                  "V") ~ "IV_V"))
  } else if (sub_ctype == "SKCM_tumor"){
    # Keep extremes II and III vs V. 
    df_pheno_of_interst <- df_pheno_of_interst %>% 
      mutate(melanoma_clark_level_value = case_when(melanoma_clark_level_value %in% c("II", 
                                                                                      "III") ~ "II_III",
                                                    melanoma_clark_level_value == "IV" ~ NA, 
                                                    TRUE ~ melanoma_clark_level_value))
    #df_pheno_of_interst <- df_pheno_of_interst %>% 
    #  mutate(melanoma_clark_level_value = case_when(melanoma_clark_level_value %in% c("II", 
    #                                                                                  "III") ~ "II_III",
...
if("menopause_status" %in% colnames(df_pheno_of_interst)){
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(menopause_status = case_when(menopause_status == "Pre_(<6_months_since_LMP_AND_no_prior_bilateral_ovariectomy_AND_not_on_estrogen_replacement)" ~ "Premenopause",
                                        menopause_status == "Post_(prior_bilateral_ovariectomy_OR_>12_mo_since_LMP_with_no_prior_hysterectomy)" ~ "Postmenopause",
                                        TRUE ~ NA))
  
  # Make a new "phenotype" which is a merge of menopause and ER: 
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(menopause_er_status = case_when(menopause_status == "Premenopause" & ER_Status_nature2012 == "Positive" ~ "Premenopause_ERpos",
                                           menopause_status == "Postmenopause" & ER_Status_nature2012 == "Positive" ~ "Postmenopause_ERpos",
                                           TRUE ~ NA))
}

# See if later in plotting remove middle (G2) for extremes...
...
if("neoplasm_histologic_grade" %in% colnames(df_pheno_of_interst)) {
  ...
  if (c_type == "CHOL"){
    df_pheno_of_interst <- df_pheno_of_interst %>% 
      mutate(neoplasm_histologic_grade = case_when(neoplasm_histologic_grade %in% c("G1", 
                                                                                    "G2") ~ "G1_G2",
                                                   TRUE ~ neoplasm_histologic_grade))  # Keep as is for all other values)
  }
}
  ...
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    ...
    number_pack_years_smoked = case_when(
      ...
      number_pack_years_smoked >= q1_packyears & 
        number_pack_years_smoked < median_packyears ~ NA, #paste0("between_", q1_packyears, "_(incl)_and_", median_packyears, "_(excl)"),
      number_pack_years_smoked >= median_packyears & 
        number_pack_years_smoked < q3_packyears ~ NA, #paste0("between_", median_packyears, "_(incl)_and_", q3_packyears, "_(excl)"),
...
# Later after plotting look at only extremes. 
# Collect stages into one: 
if("pathologic_stage" %in% colnames(df_pheno_of_interst)) {
  
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(pathologic_stage = ifelse(pathologic_stage %in% c("Stage_IA", "Stage_IB"), 
                                     "Stage_I", 
                                   ifelse(pathologic_stage %in% c("Stage_IIA", 
                                                                  "Stage_IIB", 
                                                                  "Stage_IIC", 
                                                                  "I/II_NOS"), 
                                          "Stage_II", 
                                          ifelse(pathologic_stage %in% c("Stage_IIIA",
                                                                         ...
                                                                         "Stage_IIIC"), 
                                                 "Stage_III", 
                                                 ifelse(pathologic_stage %in% c( "Stage_IVA",
                                                                                 "Stage_IVB",
                                                                                 "Stage_IVC"),
                                                        "Stage_IV", 
                                                        pathologic_stage)))))
  ...
  if (c_type == "PAAD"){
    df_pheno_of_interst <- df_pheno_of_interst %>% 
      mutate(pathologic_stage = factor(
        ifelse(ifelse(pathologic_stage %in% c("Stage_III", "Stage_IV"),
                         "Stage_III_IV", pathologic_stage))))
    
  } else if (c_type == "SKCM") {
    if (sub_ctype == "SKCM_metastatic"){
      df_pheno_of_interst <- df_pheno_of_interst %>% 
          mutate(pathologic_stage = case_when(pathologic_stage %in% c("Stage_0", 
                                                                      "Stage_I",
                                                                      "Stage_II") ~ "Stage_0_I_II",
                                              pathologic_stage %in% c("Stage_III", 
                                                                      "Stage_IV") ~ "Stage_III_IV"))
      
    } else if (sub_ctype == "SKCM_tumor"){
      df_pheno_of_interst <- df_pheno_of_interst %>% 
          mutate(pathologic_stage = case_when(pathologic_stage %in% c("Stage_I", 
                                                                      "Stage_II") ~ "Stage_I_II",
                                              pathologic_stage %in% c("Stage_III",
                                                                      "Stage_IV") ~ "Stage_III_IV"))
      
    }
  } else if (c_type %in% c("LUAD", "LUSC", "BRCA", "CHOL")){
     df_pheno_of_interst <- df_pheno_of_interst %>% 
          mutate(pathologic_stage = case_when(pathologic_stage %in% c("Stage_III",
                                                                      "Stage_IV") ~ "Stage_III_IV",

                 TRUE ~ pathologic_stage))  # Keep as is for all other values)
  }
  
}


if("residual_tumor" %in% colnames(df_pheno_of_interst)){
   df_pheno_of_interst <- df_pheno_of_interst %>% 
     mutate(residual_tumor = ifelse(residual_tumor %in% c("R1", "R2"), "R1_R2", residual_tumor))
}


# Filter post plotting! For extremes
# Change tobacco smoking history scale to uncerstandable strings: 
if("tobacco_smoking_history" %in% colnames(df_pheno_of_interst)) {
  
  if (c_type %in% c("CESC", "PAAD")){
    df_pheno_of_interst <- df_pheno_of_interst %>% 
...
# Make new phenotype (triple negative)
if (c_type == "BRCA" & all(c("HER2_Final_Status_nature2012", "PR_Status_nature2012", "ER_Status_nature2012") %in% colnames(df_pheno_of_interst))){
  
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(triple_negative = ifelse(tolower(HER2_Final_Status_nature2012) == "negative" &
                                      tolower(PR_Status_nature2012) == "negative" &
                                      tolower(ER_Status_nature2012) == "negative", TRUE, FALSE
                                      ))
}
...
# I think this was for additional treatment in SKCM. Instead I make special rule below...
# Change errors entries with pipe: 
df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(across(
        where(function(x) is.character(x) && any(grepl("\\|", x))), # Changed predicate
        ~ {
            split_vals <- str_split(.x, "\\|")  # Split the string by '|'
            # Check if all elements in the list are the same
            map_chr(split_vals, ~ if_else("YES" %in% unique(.x), "YES", 
                                           if_else("NO" %in% unique(.x), "NO", 
                                                   NA_character_)))
        }
    ))
...
# Make Negative vs Positive for receptor status (changing levels)
cols_with_receptor_status <- intersect(names(df_pheno_of_interst), c("ER_Status_nature2012", "HER2_Final_Status_nature2012", "PR_Status_nature2012"))

# Change so Low is first level and High is next: 
df_pheno_of_interst[cols_with_receptor_status] <- lapply(df_pheno_of_interst[cols_with_receptor_status], function(x) {
    if (is.factor(x)) {
        relevel(x, ref = "Positive")
    } else {
        x  # If not a factor, leave as is
    }
})
...
if("alcoholic_exposure_category" %in% colnames(df_pheno_of_interst)) {
  # alcoholic_exposure_category order: 
  level_order_alcoholic_exposure_category <- c("None", "Daily_Drinker") %>% 
    make.names
  
  df_pheno_of_interst[["alcoholic_exposure_category"]] <- factor(
    df_pheno_of_interst[["alcoholic_exposure_category"]], 
    levels=level_order_alcoholic_exposure_category)
}
...
if("primary_therapy_outcome_success" %in% colnames(df_pheno_of_interst)) {
  
  # primary_therapy_outcome_success order: 
  level_order_primary_therapy_outcome_success <- c("Complete_Remission.Response", "Partial_Remission.Response", "Stable_Disease", "Progressive_Disease") %>% 
    make.names
  
  df_pheno_of_interst[["primary_therapy_outcome_success"]] <- factor(
    df_pheno_of_interst[["primary_therapy_outcome_success"]], 
    levels=level_order_primary_therapy_outcome_success)
}

if("menopause_status" %in% colnames(df_pheno_of_interst)) {
  # menopause_status order: 
  level_order_menopause_status <- c("Premenopause", "Postmenopause") %>% 
    make.names
  
  df_pheno_of_interst[["menopause_status"]] <- factor(
    df_pheno_of_interst[["menopause_status"]], 
    levels=level_order_menopause_status)
}
...
make_all_contrasts <- function (group, des_mat=NULL, x_pheno, delim="_vs_"){
  ...
  if(!is.null(des_mat)){
    ...
    cb   <- combn(group, 2, FUN = function(x){paste0(x_pheno, x[2], "-", x_pheno, x[1])})
    contrasts<- limma::makeContrasts(contrasts=cb, levels=des_mat)
...
if (!file.exists(R_element_name)){
  ...
  for (x_pheno in phenos_to_test){
   ...
   # if (length(unique(dge_filtered$samples[[x_pheno]])) > 1){
    ...
    # Adjust for age by including it in limma model. 
    if (x_pheno != "age_at_initial_pathologic_diagnosis"){
      
      ### MAKE DESIGN AND CONTRAST MATRIX ##
      options(na.action='na.omit') # DO not include if only NAs 
      design_mat <- model.matrix(as.formula(paste("~ 0 +", x_pheno,
                                                  "+ age_at_initial_pathologic_diagnosis")),
                                  data = dge_filtered$samples)
    
      # Make contrast matrix: 
      #contrasts_mat <- make_all_contrasts(dge_filtered$samples[[x_pheno]], design_mat, x_pheno)
    ...
    df_cat_sample_size <- dge_filtered$samples[[x_pheno]] %>% 
      summary()
    names(df_cat_sample_size) <- names(df_cat_sample_size) %>% 
      paste0(x_pheno,.)
    ...
    if (length(colnames(contrasts_mat))>1){
      for (c in colnames(contrasts_mat)){
        # Update as of 13/12-2024: topTreat to topTable
        list_comparisons[[c]] <- topTable(efit, 
                  adjust.method = "fdr",
                  number = Inf,
                  coef = c) %>% 
            rownames_to_column("gene") %>% 
            filter(!str_detect(gene, "^ENSG")) %>% 
            dplyr::mutate(phenotype = x_pheno,
                   comparison = gsub(x_pheno, "", c),
      ...
      # Update as of 13/12-2024: topTreat to topTable
      list_df_logFC_pval[[x_pheno]] <- topTable(efit, 
              adjust.method = "fdr",
              number = Inf) %>% 
        rownames_to_column("gene") %>% 
        filter(!str_detect(gene, "^ENSG")) %>% 
        mutate(phenotype = x_pheno, 
              comparison = gsub(x_pheno, "", colnames(contrasts_mat)),
              total_sample_size = tot_sample_size,
              tmp_comparison = colnames(contrasts_mat)) %>% 
        mutate(cat_sample_size = paste0(unlist(str_split(tmp_comparison, "_vs_"))[1],
                                        ":",
                                        df_cat_sample_size[[unlist(str_split(tmp_comparison, "_vs_"))[1]]], 
                                        ", ", 
                                        unlist(str_split(tmp_comparison, "_vs_"))[2], 
                                        ":", 
                                        df_cat_sample_size[[unlist(str_split(tmp_comparison, "_vs_"))[2]]])) %>% 
        dplyr::select(-tmp_comparison)
...
```{r}
if(c_type == "BRCA"){
  table_w_adjP_sig <- table_w_adjP_sig %>% 
    filter(!comparison %in% c("Stage_III_IV_vs_Stage_II", "Stage_II_vs_Stage_I"))
}

if(c_type == "CESC"){
  table_w_adjP_sig <- table_w_adjP_sig %>% 
    filter(!comparison %in% c("G3_G4_vs_G2", "Former_smoker_vs_Lifelong_NonSmoker"))
}

if(c_type == "HNSC"){
  table_w_adjP_sig <- table_w_adjP_sig %>% 
    filter(!comparison %in% c("G3_G4_vs_G2", 
                              "G2_vs_G1", 
                              "Current_smoker_vs_Former_smoker_less_than_15y",
                              "Current_smoker_vs_Former_smoker_more_than_15y",
                              "Former_smoker_more_than_15y_vs_Former_smoker_less_than_15y", 
                              "Former_smoker_more_than_15y_vs_Lifelong_NonSmoker", 
                              "Former_smoker_less_than_15y_vs_Lifelong_NonSmoker",
                              "Stage_III_vs_Stage_I", "Stage_IV_vs_Stage_II", "Stage_IV_vs_Stage_III")) %>% 
    mutate(comparison = gsub("_\\._", "_", comparison)) %>% 
    mutate(comparison = gsub("_\\.", "_", comparison))
}

if(c_type == "PAAD"){
  table_w_adjP_sig <- table_w_adjP_sig %>% 
    filter(!comparison %in% c("G2_vs_G1", "G3_G4_vs_G2",
                              "Progressive_Disease_vs_Partial_Remission.Response", 
                              "Stable_Disease_vs_Complete_Remission.Response", 
                              "Stable_Disease_vs_Partial_Remission.Response",
                              "Progressive_Disease_vs_Stable_Disease", 
                              "Former_smoker_vs_Lifelong_NonSmoker"))
}

if(c_type == "LUAD"){
  table_w_adjP_sig <- table_w_adjP_sig %>% 
    filter(!comparison %in% c("Former_smoker_less_than_15y_vs_Lifelong_NonSmoker",
                              "Former_smoker_duration_na_vs_Lifelong_NonSmoker", 
                              "Progressive_Disease_vs_Stable_Disease", 
                              "Current_smoker_vs_Former_smoker_less_than_15y", 
                              "Current_smoker_vs_Former_smoker_more_than_15y", 
                              "Former_smoker_more_than_15y_vs_Former_smoker_duration_na", 
                              "Former_smoker_more_than_15y_vs_Former_smoker_less_than_15y",
                              "Former_smoker_duration_na_vs_Former_smoker_less_than_15y",
                              "Current_smoker_vs_Former_smoker_duration_na", 
                              "Former_smoker_more_than_15y_vs_Lifelong_NonSmoker"))
}

if(c_type == "LUSC"){
  table_w_adjP_sig <- table_w_adjP_sig %>% 
    filter(!comparison %in% c("Stage_II_vs_Stage_I",
                              "Stage_III_IV_vs_Stage_II",
                              "Progressive_Disease_vs_Stable_Disease", 
                              "Current_smoker_vs_Former_smoker_less_than_15y", 
                              "Current_smoker_vs_Former_smoker_more_than_15y", 
                              "Former_smoker_more_than_15y_vs_Former_smoker_duration_na", 
...
table_w_adjP_sig$comparison %>% unique()
```
...
sig_pheno_comparison_list <- table_w_adjP_sig %>% 
  filter(!grepl("cellfrac", phenotype), 
         !grepl("mut_", phenotype)) %>% 
  filter(sample_sizes_limit != "LOW") %>% 
  dplyr::select(comparison) %>% unique()
...
if (c_type == "BRCA" & nrow(sig_comparison_list) > 29){
  ...
  legend_size <- 2
} else if (nrow(sig_pheno_comparison_list) > 4 & nrow(sig_pheno_comparison_list) < 7){
  txt_axis_size <- 7
  star_size <- 3
  title_size <- 12
  subtitle_size <- 12
  legend_txt_size <- subtitle_size
  legend_size <- 5
} else if (nrow(sig_pheno_comparison_list) >= 7 & nrow(sig_pheno_comparison_list) < 10){
  txt_axis_size <- 6
  star_size <- 3
  title_size <- 12
  subtitle_size <- 12
  legend_txt_size <- subtitle_size
  legend_size <- 5
} else if (c_type == "LAML" & nrow(sig_comparison_list) > 18) {
  txt_axis_size <- 3
  star_size <- 1.5
  title_size <- 7
  ...
  legend_txt_size <- subtitle_size
  legend_size <- 3
} else if (c_type %in% c("SKCM", "CESC") & nrow(sig_comparison_list) > 14){
  txt_axis_size <- 3
  star_size <- 1
  title_size <- 5
  subtitle_size <- title_size+0
  legend_txt_size <- subtitle_size
  legend_size <- 3
} else if (c_type %in% c("PAAD", "HNSC", "LUSC") & nrow(sig_comparison_list) > 13){
  ...
  legend_txt_size <- subtitle_size
  legend_size <- 3
} else if (c_type == "SKCM" & nrow(sig_comparison_list) > 8){
  txt_axis_size <- 5
  star_size <- 2
  title_size <- 7
  subtitle_size <- title_size+0
...
star_size_mut <- 3
title_size_mut <- 12
subtitle_size_mut <- title_size+0
legend_txt_size_mut <- subtitle_size
legend_size_mut <- 6
```
...
phenotype_heatmap <- table_w_adjP_sig %>% 
  filter(!grepl("cellfrac", phenotype), 
         !grepl("mut_", phenotype)) %>% 
  mutate(phenotype = rename_phenotype[phenotype]) %>% 
  rowwise() %>% 
  ...
  mutate(condition_plot = ifelse(nchar(comparison)>50, 
                                 gsub("_vs_", "_vs\n", condition_plot),
                                 condition_plot)) %>% 
  mutate(condition_plot = gsub("Progressive_Disease_vs_Complete_Remission.Response", "Progressive_Disease_vs_Complete_Remission/Response", condition_plot)) %>% 
  mutate(condition_plot = gsub("\\.", " ", condition_plot)) %>% 
  mutate(condition_plot = gsub("_", " ", condition_plot)) %>% 
  mutate(condition_plot = gsub("_", " ", condition_plot)) %>% 
  mutate(condition_plot = gsub("\nTRUE  vs FALSE ", "", condition_plot)) %>% 
  # Remove # Histological type from y axis texts and menopause status: 
  mutate(condition_plot = ifelse(grepl("histological_type", condition),
                                 gsub("Histological Type:\n", "", condition_plot),
                                 ifelse(grepl("menopause_status", condition), 
                                        gsub("Menopause Status:\n", "", condition_plot),
                                        condition_plot))) %>%
...
#### MUTATIONS
mutation_heatmap <- table_w_adjP_sig %>% 
  filter(grepl("mut_", phenotype)) %>% 
  mutate(phenotype = paste(gsub("mut_", "", phenotype), "mutation")) %>% 
  rowwise() %>% 
  mutate(condition_plot = ifelse(comparison == "continuous_linear_relations" |
                                   grepl("MUT", comparison), 
                                 paste0(phenotype), 
  ...
  ggplot(aes(x = gene, y = condition_plot, fill = logFC)) +
  geom_tile() + 
  geom_text(aes(label = significance), color = "black", size = star_size_mut) + 
  scale_fill_gradient2(low = "blue4", mid = "white", high = "red4", midpoint = 0) +
  theme_minimal() +
  theme(axis.text.x = element_text(size = txt_axis_size_mut, 
                                   angle = 30,
                                   vjust = 0.75),
    axis.text.y = element_text(size = txt_axis_size_mut),
    axis.title = element_blank(),
    title = element_text(size=title_size_mut),
    plot.subtitle =  element_text(size=subtitle_size_mut),
    legend.title = element_text(size = legend_txt_size_mut),
    legend.text = element_text(size = legend_txt_size_mut-2),
    legend.key.size = unit(legend_size_mut, "mm")
  ) +
  #ggtitle(ifelse(c_type == "SKCM",
  #               paste0("Limma univariate analysis of ", c_type, 
  #                      " (", gsub("SKCM_", "",sub_ctype), ")"),
  #               paste0("Limma univariate analysis of ", c_type))) +
  labs(subtitle = "Mutations") + 
  coord_fixed() # Make sure tiles are square
...
## Merge phenotype + mutation plots

```{r}

txt_axis_size_mut <- 10
subtitle_size_mut <- 15

mutation_heatmap_2 <- mutation_heatmap +
  theme(legend.key.size = unit(0.5, "cm"),
        legend.title = element_text(size = txt_axis_size_mut),
        legend.text = element_text(size = txt_axis_size_mut),
        axis.text.x = element_text(size = txt_axis_size_mut, 
                                   angle = 30,
                                   vjust = 0.75),
        axis.text.y = element_text(size = txt_axis_size_mut),
        plot.subtitle =  element_text(size=subtitle_size_mut))
...
phenotype_heatmap_2 <- phenotype_heatmap +
  theme(legend.key.size = unit(0.5, "cm"),
        legend.title = element_text(size = txt_axis_size_mut),
        legend.text = element_text(size = txt_axis_size_mut),
        axis.text.x = element_text(size = txt_axis_size_mut, 
...
mut_ncol_heatmap <- mutation_heatmap_2$data$condition_plot %>% unique() %>% length()
pheno_ncol_heatmap <- phenotype_heatmap_2$data$condition_plot %>% unique() %>% length()

p1 <- egg::set_panel_size( mutation_heatmap_2, height=unit(mut_ncol_heatmap, "cm"),
                          width=unit(length(target_list), "cm") )
p2 <- egg::set_panel_size( phenotype_heatmap_2, height=unit(pheno_ncol_heatmap, "cm"),
                          width=unit(length(target_list), "cm") )

combined_plot <- cowplot::plot_grid( p1, p2 )

# Save to PDF
pdf(ifelse(c_type == "SKCM",
                  paste0(path_save_plots, sub_ctype,
                  "_heatmap_univariate_merged_mutations_phenotypes",
                  ".pdf"),
                  paste0(path_save_plots, c_type,
                  "_heatmap_univariate_merged_mutations_phenotypes",
                  ".pdf")), 
    width = 16.1, height = 6) # Adjust width and height as needed
print(combined_plot)
dev.off()
```
...
sig_genes <- df_sig_genes_univariate %>% 
...
if (include_cell_fractions){
  ...
  sig_pheno_2 <- sig_phenotypes_univariate
}
...
for (x_pheno in sig_pheno_2){
  ...
  if(length(sig_genes_xpheno) >0){
    ...
    colnames(x_pheno_tpm_data)[4] <- "Subgroup"
...
```{r}
# Function to rename cat_sample_size to not include phenotype
rm_phenotype_string <- function(str1, str2) {
  # Split str1 by '_vs_' and extract the categories
  categories <- str_split(str1, "_vs_")[[1]]
  cat_sample_sizes <- str_split(str2, ", ")[[1]]
  
  # Extract the number for category:  
  # For FEMALE and MALE MALE is in FEMALE, therefore I need to do: 
  if (sum(grepl(categories[1], cat_sample_sizes))> 1 | sum(grepl(categories[2], cat_sample_sizes))>1){
     return(paste0(categories[1], str_extract(cat_sample_sizes[1], ":\\d+"), ", ", 
                   categories[2], str_extract(cat_sample_sizes[2], ":\\d+")))
  } else {
     return(paste0(categories[1], str_extract(cat_sample_sizes[grepl(categories[1], cat_sample_sizes)], ":\\d+"), ", ", 
                   categories[2], str_extract(cat_sample_sizes[grepl(categories[2], cat_sample_sizes)], ":\\d+")))
  }
}
...
univariate_sample_size_table <- table_w_adjP_sig %>% 
  ...
  #mutate(phenotype = gsub("Tregs", "", phenotype)) %>% 
  rowwise() %>% 
  mutate(cat_sample_size = rm_phenotype_string(comparison, cat_sample_size)) %>% 
  # Change age to use symbols: 
