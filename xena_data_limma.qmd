...
# Empty list to save efit element for all the phenotypes with significant target genes:
list_efit_sig <- list()
...
for (x_pheno in phenos_to_test){
  ...
  if (nrow(results) < 1){
    ...
    # Save efit for the phenotype with significant target genes: 
    list_efit_sig[[x_pheno]] <- efit
  }
...
```{r}
# Make table with number of individuals in each category
...
topTreat(list_efit_sig$histological_type, 
        adjust.method = "fdr",
        genelist = target_list)
  
```

Make plots with the results.
