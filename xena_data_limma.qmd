...
if("alcoholic_exposure_category" %in% colnames(df_pheno_of_interst)) {
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(alcoholic_exposure_category = factor(
      ifelse(alcoholic_exposure_category %in% c("Occasional Drinker", 
                                                "Social Drinker"), 
...
if("pathologic_stage" %in% colnames(df_pheno_of_interst)) {
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    mutate(pathologic_stage = factor(
      ifelse(pathologic_stage %in% c("Stage I", "Stage IA", "Stage IB"), 
         "Stage_I", 
         ifelse(pathologic_stage %in% c("Stage IIA", "Stage IIB"),
                "Stage_II", 
                ifelse(pathologic_stage %in% c("Stage III", "Stage IV"),
...
```{r}
## Function for making design and contrast matrices: 
setup_design_contrasts <- function(pheno_of_int, return_table){
  
  print(pheno_of_int)
  
  categories <- dge$samples[[pheno_of_int]] %>% unique()
  ...
  ### CONTRAST MATRICES: ###
  if (pheno_of_int == "alcoholic_exposure_category"){
  ...
  if (pheno_of_int == "histological_type"){
    contrasts_mat <- makeContrasts(
  ...
  if (pheno_of_int == "neoplasm_histologic_grade"){
    contrasts_mat <- makeContrasts(
  ...
  if (pheno_of_int == "pathologic_stage"){
    contrasts_mat <- makeContrasts(
  ...
  if (pheno_of_int == "tobacco_smoking_history"){
    contrasts_mat <- makeContrasts(
  ...
  if (return_table == "design") {
    return(design_mat)
  } else if(return_table == "contrast") {
...
for (x_pheno in phenos_to_test){
  ...
  design_mat <- setup_design_contrasts(pheno_of_int = x_pheno,
                                       return_table = "design")
  contrasts_mat <- setup_design_contrasts(pheno_of_int = x_pheno,
                                       return_table = "contrast")
  
  print("Done with design, contrast matrices")
  ...
  print("Done with dge filtering on samples and genes")
  ...
  print("Done with finding significantly expressed genes")
  ...
  print("Done with adding results to table")
