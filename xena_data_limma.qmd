...
min_sample_size <- ifelse(c_type == "CHOL", 5, 10)
```
...
if ("age_at_initial_pathologic_diagnosis" %in% colnames(df_pheno_of_interst)){
  ...
  df_pheno_of_interst <- df_pheno_of_interst %>% 
    ...
    age_at_initial_pathologic_diagnosis = case_when(
      ...
      age_at_initial_pathologic_diagnosis >= q3_age ~  paste0("less_than_",
                                                              q3_age)
    )
  )
} 
...
#stop("Continue here")
##### NB!  ! ! ! ! ! USE MEDIAN, QUARENTILES AS SUGGESTED BY CHAT GPT #####
...
table_w_adjP_sig %>% 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(size = ifelse(c_type %in% c("BRCA", "LAML"), 
                                                 6, 12), 
                                   angle = 30,
                                   vjust = 0.75),
    axis.text.y = element_text(size = ifelse(c_type %in% c("BRCA", "LAML"), 
                                             8, 12)),
...
comparison <- important_phenos
df <- df_cat_sample_size
